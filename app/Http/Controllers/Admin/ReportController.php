...
class ReportController extends Controller
{
    ...
    public function index(Request $request): View|RedirectResponse
    {
        ...
        $reportData = $this->buildAdminReportsData($request);

        return view('admin.reports', array_merge(RouteHelpers::pageMeta('reports'), $reportData));
    }

    /**
     * Display a specific report detail or trigger Excel export.
     */
    public function show(Request $request, string $reportSlug): View|Response|RedirectResponse
    {
        if ($redirect = RouteHelpers::ensureAdmin()) {
            return $redirect;
        }
        ...
        if ($request->boolean('export')) {
            return $this->buildExcelResponse($selectedReport, $reportData);
        }
    ...
    /**
     * Export all members data to Excel format.
     */
    public function exportMemberData(): Response|RedirectResponse
    {
        if ($redirect = RouteHelpers::ensureAdmin()) {
            return $redirect;
        }

        return $this->buildMemberExportResponse();
    }
    ...
    /**
     * Helper to build report statistics and catalog.
     */
    protected function buildAdminReportsData(Request $request): array
    {
        ...
        $vitaminProductRecords = Product::query()
            ...
            ->orderBy('name')
            ->get();
        $stockLogRecords = ProductStockLog::with(['product', 'user'])->latest()->get();
        ...
        $reportCatalog = collect([
            ...
            $dummyDetails = match ($report['slug']) {
                ...
                'laporan-stok-barang' => [
                    'columns' => ['Waktu / Produk', 'Brand / Item', 'Tipe / Status', 'Jumlah / Stok', 'Harga / Keterangan', 'Oleh / Petugas'],
                    'rows' => (function () use ($stockLogRecords, $formatMoney) {
                        $logRows = $stockLogRecords->map(fn($log) => [
                        ...
                        $productRows = $allProducts->map(fn($p) => [
                        ...
                        return array_merge($logRows, $productRows);
                    })(),
    ...
    /**
     * Build Excel XML table response.
     */
    protected function buildExcelResponse(array $selectedReport, array $reportData): Response
    {
    ...
    /**
     * Build members export response.
     */
    protected function buildMemberExportResponse(): Response
    {
